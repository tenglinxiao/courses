  ...
  public void execute(StagedOutputJobExecutor executor) throws IOException, InterruptedException, ExecutionException
  {
    ...
    for (FileStatus clusterPath : clusterPaths)
    {
      ...
      for (FileStatus yearPath : yearPaths)      
      {
        ...
        FileStatus[] monthPaths = _fs.listStatus(yearPath.getPath());
        for(FileStatus monthPath : monthPaths)
        {
            String month = monthPath.getPath().getName();
            FileStatus[] dayPaths = _fs.listStatus(monthPath.getPath());
            for (FileStatus dayPath : dayPaths)
            {
              ...
              String outputPathForDay = String.format("%s/%s/%s/%s/%s",_usageOutputPathRoot,clusterName,year,month,day);
