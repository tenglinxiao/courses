...
public class JobStatsProcessing
{
  ...
  {    
    ...
    { 
      ...
      String pathFormat = String.format("%s/%s/%s/%s/%s/*/*%s",logsRoot,clusterName,yearFormat.format(date),monthFormat.format(date),dayFormat.format(date), suffix);
      System.out.println("PathFormat: " + pathFormat);
      FileStatus[] stats = null;
      if(!suffix.equals(""))
      {
          stats = fs.globStatus(new Path(pathFormat));
      }
      else {
          stats = fs.globStatus(new Path(pathFormat), new PathFilter(){
      ...
      System.out.println("Found " + stats.length + " paths to process");
